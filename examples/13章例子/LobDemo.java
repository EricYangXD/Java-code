...
public class LobDemo {
	public static void main(String[] args) {
		...
		try {
			...
			// 填入数据库
			pstmt = conn.prepareStatement("INSERT INTO t_file VALUES(?, ?, ?)");
			pstmt.setInt(1, 1);
		...
		try {
			...
			ResultSet result = stmt.executeQuery("SELECT * FROM t_file");
			result.next();
			String filename = result.getString(2);
			Blob blob = result.getBlob(3);
			// 写入文件，文件名附加.bak
			FileOutputStream fout = new FileOutputStream(filename + ".bak");
